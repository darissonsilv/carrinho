

<?php
require_once '../config/config.php';
require_once '../models/produto.php';

// Recebe os dados do formulário de edição de produto
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $produto_id = $_POST['produto_id'];
    $nome = $_POST['nome'];
    $valor = $_POST['valor'];

    // Realize aqui a validação dos dados recebidos, se necessário.

    // Atualiza o produto no banco de dados
    $produto = new Produto($nome, $valor);
    $produto->setId($produto_id); // Define o ID do produto a ser atualizado
    $produto->atualizar(); // Método para atualizar o produto no banco

    // Redireciona para a página de listagem de produtos
    header('Location: /views/admin/listar_produto.php');
    exit;
}
?>
